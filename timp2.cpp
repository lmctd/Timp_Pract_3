...
int main()
{
    ...
    selfAddr->sin_addr.s_addr = 0;  //адреса компьютера
    ...
    strcpy(buf,"Hello world!");   //копирование строки
    int msgLen = strlen(buf);           //вычисление длины строки

    // создание сокета
    int mySocket = socket(AF_INET, SOCK_DGRAM, 0); //протокол udp
    if (mySocket == -1) {
        close(mySocket);
